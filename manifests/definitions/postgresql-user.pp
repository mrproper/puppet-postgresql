    ...
    present: {
      ...
      if $password {
        ...
        exec { "Change password for postgres user $name":
          ...
          user    => "postgres",
          unless  => "echo && TMPFILE=$(mktemp /tmp/.pgpass.XXXXXX) && echo '${host}:${port}:template1:${name}:${pgpass}' > \$TMPFILE && PGPASSFILE=\$TMPFILE psql -h ${host} -p ${port} -U ${name} -c '\\q' template1 && rm -f \$TMPFILE",
          require => [User["postgres"], Exec["Create postgres user $name"]],
        }
      }
